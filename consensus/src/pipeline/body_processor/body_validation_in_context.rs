...
impl BlockBodyProcessor {
    ...
    fn check_block_transactions_in_context(
        self: &Arc<Self>,
        block: &Block,
    ) -> BlockProcessResult<()> {
    ...
    fn check_parent_bodies_exist(self: &Arc<Self>, block: &Block) -> BlockProcessResult<()> {
        ...
        if block.header.direct_parents().len() == 1
            && block.header.direct_parents()[0] == self.genesis.hash
        {
            return Ok(());
        }
    ...
    fn check_coinbase_blue_score_and_subsidy(
        self: &Arc<Self>,
        block: &Block,
    ) -> BlockProcessResult<()> {
        match self
            ...
            .deserialize_coinbase_payload(&block.transactions[0].payload)
        {
            Ok(data) => {
                if data.blue_score != block.header.blue_score {
                    return Err(RuleError::BadCoinbasePayloadBlueScore(
                        data.blue_score,
...
mod tests {
    ...
    async fn validate_body_in_context_test() {
        ...
        consensus
            .add_block_with_parents(1.into(), vec![config.genesis.hash])
            .await
            .unwrap();
        ...
        let valid_block = consensus.build_block_with_parents_and_transactions(
            3.into(),
            vec![config.genesis.hash],
            vec![],
        );
        consensus
            .validate_and_insert_block(valid_block.to_immutable())
            .virtual_state_task
            .await
            .unwrap();
        {
            let mut block = consensus.build_block_with_parents_and_transactions(
                2.into(),
                vec![3.into()],
                vec![],
            );
        ...
        let valid_block_child =
            consensus.build_block_with_parents_and_transactions(6.into(), vec![3.into()], vec![]);
        consensus
            .validate_and_insert_block(valid_block_child.clone().to_immutable())
            .virtual_state_task
            .await
            .unwrap();
        {
